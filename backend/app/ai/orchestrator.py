...
def extract_text(content) -> str:
    if isinstance(content, list):
        return " ".join(
            [str(item.get("text", "")) for item in content if isinstance(item, dict) and "text" in item]
        )
...
def execute_tools(response, tool_map):
    ...
    for tc in response.tool_calls:
        ...
        try:
            ...
            tool_messages.append(ToolMessage(content=f"Error: {str(e)}", tool_call_id=tc["id"]))
...
async def _run_agent_loop(state: State, bound_llm, base_prompt: str, tool_map: dict, max_iterations: int = 6):
    ...
    for i in range(max_iterations):
        ...
        tool_msgs = execute_tools(response, tool_map)
